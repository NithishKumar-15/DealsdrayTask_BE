...
const employeeLoginController = async (req, res) => {
    try {
        ...
        //if the user availabe compare the password 
        if (data != null) {
            const comparePwd = await bcrypt.compare(req.body.password, data.f_Password);

            //if the password is correct send message Login successfull else send password incorrect
            if (comparePwd) {
